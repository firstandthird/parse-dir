...
class FileProcessor {
  ...
  __determineIfDirectory (done, results) {
  ...
  __getFileContents (done, results) {
  ...
  __parseFileContents (done, results) {
  ...
  processFile (file, callback) {
    ...
    async.auto({
      ...
      isDirectory: ['meta', '__async', this.__determineIfDirectory],
      fileContents: ['isDirectory', this.__getFileContents],
      parsedContents: ['fileContents', this.__parseFileContents]
    },
    (err, results) => {
...
// front-end to parse a directory of
class ParseDir {
  constructor(options) {
    this.__async = options.async !== undefined ? options.aync : true;
